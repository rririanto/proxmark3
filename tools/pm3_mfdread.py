...
def accbits_for_blocknum(accbits_str, blocknum):
    ...
    inverted.invert()
    return bits if bits.bin == inverted.bin else False
...
def print_info(data):
    ...
    for c in range(len(blocksmatrix)):
        sector_size = len(blocksmatrix[c]) - 1

        # Fill in the access bits
        blockrights[c] = accbit_info(
            BitArray(f'0x{blocksmatrix[c][sector_size][12:20]}'), sector_size
        )


        # Prepare colored output of the sector trailor
        keyA = bashcolors.RED + blocksmatrix[c][sector_size][:12] + bashcolors.ENDC
    ...
    print("\n\tUID:  " + blocksmatrix[0][0][:8])
    print("\tBCC:  " + blocksmatrix[0][0][8:10])
    print("\tSAK:  " + blocksmatrix[0][0][10:12])
    print("\tATQA: " + blocksmatrix[0][0][12:14])
    print(
        f"                   {bashcolors.RED}Key A{bashcolors.ENDC}    {bashcolors.GREEN}Access Bits{bashcolors.ENDC}    {bashcolors.BLUE}Key B{bashcolors.ENDC}"
    )
    ...
    for q in range(len(blocksmatrix)):
        print("╠═════════╬═══════╬══════════════════════════════════╬════════╬═════════════════════════════════════╣")
        n_blocks = len(blocksmatrix[q])

        # z is the block in each sector
        for z in range(len(blocksmatrix[q])):
            ...
            if isinstance(blockrights[q][z], BitArray):
                ...
                accbits = f"{bashcolors.WARNING}ERR{bashcolors.ENDC}"
